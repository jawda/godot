...
func _physics_process(delta: float) -> void:
	...
	#check to see if sprite hit ground
...
func player_interact() -> void:
	#pick up one item only if near more
...
func throw() -> void:
	throwable.get_parent().remove_child( throwable )
	PlayerManager.player.get_parent().call_deferred( "add_child", throwable )#call during next idle time

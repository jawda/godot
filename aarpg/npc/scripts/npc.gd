@tool
@icon( "res://npc/icons/npc.svg" )#this is just setting up the icon
...
func _on_interaction_finished() -> void:
	#conversation over go back to life
	state = "idle"
	update_animation()
	do_behavior = true
	do_behavior_enabled.emit()
...
func update_direction( target_position : Vector2 ) -> void:
	...
	if direction_name == "side" and direction.x < 0:#flip sprite if facing left
		sprite.flip_h = true
	else:
		sprite.flip_h = false

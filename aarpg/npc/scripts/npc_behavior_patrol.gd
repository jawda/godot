...
func start() -> void:
	...
	if has_started == true:
		#time left we should still be in the idle phase so return
		if timer.time_left == 0:
			walk_phase() #otherwise npc was walking return to walking
		return
...
func idle_phase() -> void:
	...
	var wait_time : float = target.wait_time #get to target and wait for commands
...
func walk_phase() -> void:
	#got to location waited time to walk to next
	npc.state = "walk"

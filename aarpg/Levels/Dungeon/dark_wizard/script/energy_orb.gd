...
func _ready() -> void:
	...
	get_tree().create_timer( 5 ).timeout.connect( destroy )#vanish after 5 seconds
	direction = global_position.direction_to( PlayerManager.player.global_position )
	flicker()
	pass
	
func _process(delta: float) -> void:
	#move towards player where direction set on ready
	position += direction * speed * delta
	pass

func flicker() -> void:
	modulate.a = randf() * 0.7 + 0.3 #make orb flicker

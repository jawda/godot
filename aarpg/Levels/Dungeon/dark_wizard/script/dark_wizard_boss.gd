...
const ENERGY_BALL_SCENE : PackedScene = preload("res://Levels/Dungeon/dark_wizard/energy_orb.tscn")
...
var audio_shoot : AudioStream = preload("res://assets/wizard_boss/boss_fireball.wav")

var current_position : int = 0
var positions : Array[ Vector2 ]
var beam_attacks : Array[ BeamAttack ]

var damage_count : int = 0 #times hit since last teleport
...
func _ready() -> void:
	...
	if persistent_data_handler.value == true:#is boss dead?
		door_block.enabled = false
		queue_free()
		return
		
	hp = max_hp #reset boss on enter
...
func idle() -> void:
	enable_hit_boxes()
	#random chance to just launch attack
...
func energy_beam_attack() -> void:
	...
	for b in _b: 
		beam_attacks[ b ].attack() #special beam canon!
